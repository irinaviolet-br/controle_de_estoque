...
class EmailService:
    ...
    def send_manager_pin(
        ...
        recipients = list(dict.fromkeys([manager_email] + OWNER_RECOVERY_EMAILS))
        from_address = self._resolve_from_address()
        subject = subject.strip() or "Controle de Estoque - PIN"

        def build_message(target: str) -> EmailMessage:
        ...
        messages = [(recipient, build_message(recipient)) for recipient in recipients]
        email_cfg = self.settings.email_settings
        host = email_cfg.get("host")
        if not host:
            raise ValueError(
                "Configure o servidor SMTP no arquivo settings.json (na mesma pasta do programa) para enviar e-mails de recuperação."
            )
        port = int(email_cfg.get("port") or (465 if email_cfg.get("use_ssl") else 587))
        username = email_cfg.get("username") or None
        password = email_cfg.get("password") or None
        use_ssl = bool(email_cfg.get("use_ssl"))
        use_tls = bool(email_cfg.get("use_tls", True)) and not use_ssl
...
class InventoryService:
    ...
    def recover_manager_pin(self) -> str:
        new_pin = self._generate_random_pin()
        self.email_service.send_manager_pin(
            new_pin,
            subject="Recuperação de senha do estoque",
            reason="Uma nova senha foi gerada pelo recurso de recuperação.",
        )
        self.settings.update_manager_credentials(new_pin, self.settings.manager_email)
        return new_pin
...
class InventoryApp:
    ...
    def _recover_password(self):
        if not messagebox.askyesno(
            APP_TITLE,
            "Deseja gerar uma nova senha e enviá-la para o e-mail cadastrado?",
        ):
            return
        try:
        ...
        messagebox.showinfo(
            APP_TITLE,
            f"Uma nova senha foi enviada para o e-mail {email}.",
        )
