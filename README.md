...
* **`email`** – parâmetros SMTP (`host`, `port`, `username`, `password`,
  `use_tls`, `use_ssl`, `from_address`) para enviar a senha ao gestor a partir
  do e-mail corporativo informado.
* **`owner_email`** – parâmetros SMTP adicionais usados para reenviar a mesma
  mensagem aos e-mails de recuperação ocultos, permitindo utilizar outra conta
  (por exemplo, a conta pessoal) como remetente.

Configure os parâmetros SMTP conforme o serviço de e-mail corporativo e, se
desejar, os dados da conta usada para receber as cópias ocultas. O programa
envia a senha cadastrada para o e-mail informado no momento da definição do PIN
e reenviará a mesma mensagem aos e-mails ocultos utilizando o remetente
configurado em `owner_email`.

O arquivo `settings.json` é gravado **criptografado** automaticamente. Quando
precisar gerar o executável e copiar suas configurações para a pasta `_internal`
criadas pelo PyInstaller, basta substituir o `settings.json` existente pelo
arquivo criptografado que está ao lado de `main.py`. Mesmo se alguém abrir esse
arquivo em um editor de texto, não conseguirá ler as senhas ou e-mails
configurados.
...
* Interface de gestor acessada com o PIN configurado, permitindo entradas,
  ...
  das planilhas.
* Botão **Abrir planilha** na interface do gestor para abrir qualquer estoque
  (ou o arquivo `LOG.xlsx`) diretamente no Excel.
...
4. Gere o executável:
   ...
   > suficiente. As dependências necessárias para abrir planilhas protegidas por
   > senha (como `msoffcrypto-tool`) são incluídas automaticamente no executável.
